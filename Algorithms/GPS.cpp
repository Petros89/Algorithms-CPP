...
private:
   ...
   typedef vector<c_t*> q_t;

   unordered_map<int,c_t> m;
   q_t q;
   ...
   void insert(q_t& v, c_t* value)
   {
      ...
      q_t::iterator it = lower_bound(v.begin(),v.end(), value, [](const c_t* l, const c_t* r) {return l->d > r->d;});
      v.insert(it,value);
   }
...
public:
   ...
   **/
   void addOrUpdate(int cityId, int distance)
   {
      ...
      else
      {
         ...
         auto lower = lower_bound(q.begin(),q.end(),c.d,[](const c_t* c,int d) {return d < c->d; });

         size_t i = std::distance(q.begin(), lower);
         ...
         {
            auto nl = upper_bound(q.begin() + i ,q.end(),distance,[](int d, const c_t* c) {return c->d < d; });
            size_t ni = std::distance(q.begin(), nl);

            rotate(q.begin() + i, q.begin() + i + 1, q.begin() + ni);
         }
         else
         {
            auto nl = upper_bound(q.begin(), q.begin() + i + 1,distance,[](int d, const c_t* c) {return c->d < d; });
            size_t ni = std::distance(q.begin(), nl);

            rotate(q.begin() + ni,q.begin() +  i,q.begin() +  i + 1);
         }
   ...
   **/
   int pollClosestCity()
   {
      ...
      cout << "[Min Distance [KM] | Through City] <==> [ " <<  cdist << " | ";
      return cid;

   }

   template <typename Iterator>
   void rotate_left(Iterator begin, Iterator end, size_t n)
   {
   ...
   template <typename Iterator>
   void rotate_right(Iterator begin, Iterator end, size_t n)
   {
